...
This is a Laravel 12 application. The central + tenant-schema database (see "Database architecture" below), Sanctum SPA auth (customer self-registration, plus self-service store-signup that creates a `pending` tenant awaiting admin approval), role-gated admin CRUD for tenants/business types, tenant provisioning, public tenant discovery, a tenant-scoped product/service catalog (soft-deletable), order/booking placement, order/booking/payment fulfillment (status transitions, mark-paid), a standalone paginated payments listing, per-tenant analytics (revenue, order/booking status breakdowns, top products/services), staff management, service-time-slot scheduling (with per-slot staff eligibility enforced on bookings), and a "which tenants can I access" lookup for the business portal are built. Check `database/migrations/`, `app/Models/`, and `routes/api.php` for the current state rather than assuming this doc stays in sync.

Known gaps as of this writing: no image upload endpoint for product *variants* specifically (there's no `ProductVariantController` yet to create a variant against in the first place — products/services image upload is built); no automated test coverage beyond Laravel's default skeleton tests — everything so far has been verified via manual end-to-end curl testing during development, not a repeatable suite. Also: a product/service's `slug` has a plain (not partial/conditional) unique DB constraint, so a new product can't reuse a soft-deleted one's slug — retiring and replacing a product under the same slug isn't possible right now.
...
- `App\Http\Controllers\Api\AuthController`: `register` (always creates a `customer`-role account — admin/staff accounts are never self-service), `login`, `logout`, `me`. Frontend flow: `GET /sanctum/csrf-cookie` first, then `POST /api/register` or `/api/login` with the `X-XSRF-TOKEN` header (axios does this automatically with `withCredentials: true`). Note for manual/curl testing: `EnsureFrontendRequestsAreStateful` only starts a session (and only then does `$request->session()` work) when the request's `Origin`/`Referer` matches a `SANCTUM_STATEFUL_DOMAINS` entry — a bare curl request with no `Origin` header hits `RuntimeException: Session store not set on request.` on the `session()->regenerate()` call, even though validation/creation already succeeded. Always send `-H "Origin: http://localhost:5173" -H "Referer: http://localhost:5173/"` (or whatever `FRONTEND_URLS`/`SANCTUM_STATEFUL_DOMAINS` is set to) when curl-testing any endpoint that logs the user in.
- `App\Http\Controllers\Api\StoreSignupController` (`POST /api/store-signup`, public): the second self-service entry point besides plain customer `register` — one combined submission creates the applicant's account (identical customer-role account `register` would produce) *and* a `status: pending` `Tenant` owned by them (`owner_user_id`), provisioning its Postgres schema immediately (same as `Admin\TenantController::store`) so it's ready the moment an admin approves it. Approval is nothing new — it's just an admin calling the existing `PATCH /api/admin/tenants/{tenant}` with `{status: active}`; this endpoint deliberately has no way to set status itself. On provisioning failure, both the tenant row and the just-created user are deleted (the whole signup fails together, not a half-created account). `GET /api/business-types` (public, reuses `Admin\BusinessTypeController@index`) exists solely so the store-signup form's business-type dropdown has something to populate before any tenant exists — `TenantDiscoveryController` can't help here since it only surfaces business types already attached to an active tenant.
- RBAC: `App\Http\Middleware\EnsureUserHasRole` (aliased `role`) checks `$user->role->slug` against the roles passed to the middleware, e.g. `role:admin`. The `admin/*` route group (`BusinessTypeController`, `TenantController`) runs `['auth:sanctum', 'role:admin']`. This is global-role gating only — it doesn't know about `tenant_staff` membership/tenant-scoped roles, which is handled separately (see `Tenant::isManagedBy`/`isAdministeredBy` in "Tenant-scoped requests").
- `App\Http\Controllers\Api\MyTenantsController` (`GET /api/my-tenants`, `auth:sanctum`, not tenant-scoped): the business portal's entry point — which tenants the logged-in user can open, since every other tenant-scoped route requires already knowing the tenant's slug and there's no reverse lookup anywhere else. Merges tenants owned by the user (`owner_user_id`) with tenants they have an active `tenant_staff` row for, shaped to `{id, name, slug, status, business_type, role, is_admin}` where `role` is `owner`/`admin`/`staff` and `is_admin` collapses "owner" and "tenant_staff role=admin" into one flag (both satisfy `isAdministeredBy` identically, so the frontend only needs the one boolean to decide whether roster management is available).
- **Frontend gotcha, cost real debugging time**: any component that fires a stateful API call (even a plain public `GET`) from a `useEffect` on mount will, under React 19 `StrictMode` in dev, fire that effect twice — two concurrent first-touch requests race to establish a session before either has a cookie yet, and the result is a corrupted session/CSRF-token pairing: every request in the sequence (including a subsequent `POST /api/register` or `/api/store-signup`) mints its *own* fresh `sessions` row instead of reusing one, and the real submission 419s with "CSRF token mismatch" even though the token/cookie the browser sent look individually valid. Symptom to recognize it by: `SELECT COUNT(*) FROM sessions` grows by one per request in the sequence, not one per browser tab. Fix is the standard React `AbortController`-per-effect pattern (see `frontend/src/pages/SignUp.jsx`'s `business-types` fetch) — abort in the cleanup function so only the real (second) mount's request actually completes — not disabling `StrictMode`.
...
- `App\Http\Controllers\Api\Tenant\TenantStaffController` (`/staff`): adds an already-registered user as staff directly — no separate invite-and-accept step, `store` just creates the `tenant_staff` row (`isAdministeredBy`-gated). `index` is `isManagedBy`-gated (broader — a plain staff member can see coworkers, not edit the roster). `GET .../staff/lookup?email=` (`isAdministeredBy`-gated, same as `store`) resolves an email to `{id, name, email}` so the "add staff" form has a `user_id` to submit — users aren't publicly searchable otherwise, and `store` needs the id, not an email.
...
- Both `store` methods `lockForUpdate()` the row the capacity/stock check depends on (`Product`/`ProductVariant` per line item; `Service` once, before its capacity count) *inside* the transaction, before checking — otherwise two simultaneous requests can both read the pre-write count, both pass, and both write, oversubscribing by however many requests raced. Verified under real concurrency, not just sequentially: 5 parallel requests for a stock-1 product / capacity-1 service each produced exactly 1 success and 4 clean 422s, with stock landing at exactly 0.
...
- `GET .../payments` (`PaymentController::index`, manager-only via `isManagedBy` — there's no "my payments" case to carve out the way orders/bookings do, a payment is never browsed except by whoever runs the tenant): optional `?status=paid|unpaid` filter, paginated. `payable` (the underlying order or booking) is eager-loaded per-type via `MorphTo::morphWith([Order::class => ['user'], Booking::class => ['user', 'service']])` so the table can show what each payment is actually for without an extra request per row.
- `GET .../analytics` (`AnalyticsController`, manager-only via `isManagedBy`): last-30-days paid-revenue trend (grouped by day), total paid/unpaid revenue, order/booking counts by status, and top 5 products/services (products by revenue via `order_items`, services by booking count). The grouped-aggregate queries go through `DB::connection('tenant')->table(...)` directly rather than the Eloquent models — running `selectRaw`/`groupBy` results back through a model's casts turns `status` into an enum instance, which is awkward as an array key (`pluck('count', 'status')`), and none of these aggregates need model hydration anyway.
- Every tenant-scoped and admin list endpoint (`products`, `services`, `orders`, `bookings`, `staff`, `payments`) is paginated (`paginate()`, default 15 per page, override with `?per_page=`) rather than returning a flat array — this changed for all of them together, so there's no endpoint still returning an unpaginated list to special-case on the frontend.

## Image uploads

- `cloudinary-labs/cloudinary-laravel` (installed, config published). **Important quirk**: despite the package name, its `Cloudinary` singleton is bound from `config('filesystems.disks.cloudinary.url')` — *not* `config('cloudinary.cloud_url')`, even though `config/cloudinary.php` exists and looks like the obvious place. `config/filesystems.php` has a `cloudinary` disk added (`'url' => env('CLOUDINARY_URL')`) specifically to satisfy this; skipping it throws `Cloudinary\Exception\ConfigurationException` regardless of `CLOUDINARY_URL` being set correctly. `CLOUDINARY_URL` (format `cloudinary://<api_key>:<api_secret>@<cloud_name>`, from the Cloudinary dashboard's "API Environment variable") lives only in `.env` (not `.env.example`, which just documents the format in a comment) since it's a real secret.
- `App\Services\CloudinaryUploader`: thin wrapper around `Cloudinary::uploadApi()->upload()`/`->destroy()`, returning/taking `{url, public_id}` — `public_id` is what `destroy()` actually needs; it can't be reliably derived back out of a URL, so every image table (`product_images`, `service_images`, `product_variant_images`) stores it alongside `url`.
- `App\Http\Controllers\Api\Tenant\ProductImageController` / `ServiceImageController` (`POST .../products/{product}/images`, `DELETE .../products/{product}/images/{image}`, and the `services` equivalent): `tenant.access`-gated, same as other catalog writes. `store` enforces the max-4-per-product/service rule app-side (counts existing images, 422s at the limit) before uploading — no DB constraint for it. `destroy` calls Cloudinary's `destroy()` before deleting the DB row, so an image can't be orphaned on Cloudinary's side. Verified end-to-end against a real Cloudinary account: upload, 5th-image rejection, and delete confirmed gone from both Postgres and Cloudinary's Admin API.
- No `ProductVariantImageController` yet — variants have no creation endpoint at all yet (`ProductVariantController` doesn't exist), so there's nothing to attach a variant image to. The `product_variant_images` table and `public_id` column exist and are ready for whenever that gets built.

## Notifications and scheduled jobs

- Laravel's built-in **database** notification channel (`notifications` table, central schema — `User` already has `Notifiable` from the framework skeleton) — not real push (no device tokens/APNs/FCM involved) and not queued (`ShouldQueue` deliberately omitted): each notification is sent synchronously, in the same request/command tick that caused it, while the `tenant` connection is still correctly pointed at the right schema. `GET /api/notifications` (`{unread_count, notifications}`), `PATCH /api/notifications/{id}/read`, `POST /api/notifications/read-all` — all under plain `auth:sanctum`, not tenant-scoped, since one user's notifications span every tenant they've touched.
- Three notifications exist so far, all customer/staff-facing: `OrderStatusChanged` and `BookingStatusChanged` (sent from `OrderController`/`BookingController::update()`, and from the scheduled command below), and `AddedAsTenantStaff` (sent from `TenantStaffController::store()`). Each snapshots the relevant fields (status, tenant name/slug, amount, etc.) into `toArray()` rather than storing a live model reference — correct given they're synchronous, but would need revisiting if these ever became queued (the tenant connection wouldn't still be pointed correctly by the time a queued job ran).
- `App\Services\OrderCancellationService::cancel()` — extracted the "restore stock for tracked items, then mark cancelled" logic out of `OrderController::update()` so the scheduled command's stale-order cleanup can reuse the exact same code path instead of duplicating it (and risking the two drifting apart).
- `App\Console\Commands\ProcessTenantSchedules` (`tenants:process-schedules`, scheduled hourly in `routes/console.php` via `Schedule::command(...)->hourly()`) iterates every `status = active` tenant, points the `tenant` connection at each in turn (same `Config::set(...) + DB::purge('tenant')` technique as `TenantProvisioner`/`ResolveTenant`), and: completes `confirmed` bookings whose `ends_at` has passed, cancels `pending` orders older than 24h (via `OrderCancellationService`, so stock gets restored), and cancels `pending` bookings older than 24h. All three send the relevant status-changed notification. Verified idempotent (a second immediate run finds nothing left to process) and verified across a real auto-complete + auto-cancel + stock-restore + notification chain end-to-end.
- **Timestamp comparison gotcha, cost real debugging time**: `starts_at`/`ends_at`/etc. are `timestamp without time zone` columns, and `config('app.timezone')` is `UTC` — the application (Carbon, Eloquent) is internally consistent because it always reads and writes in UTC. Backdating a timestamp for a manual test via raw `psql` using bare `now() - interval '...'` uses the **Postgres session's local timezone** instead (e.g. `+03`), silently writing a value that isn't actually in the past relative to what the app considers "now" (UTC) — the scheduled command then correctly finds nothing to process, which looks like a bug in the command but is actually a bug in the test setup. Fix: `(now() AT TIME ZONE 'UTC') - interval '...'` when backdating manually.
