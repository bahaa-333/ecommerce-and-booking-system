...
This is a Laravel 12 application. The central + tenant-schema database (see "Database architecture" below), Sanctum SPA auth, role-gated admin CRUD for tenants/business types, tenant provisioning, a tenant-scoped product/service catalog, and order/booking placement (price snapshotting, stock decrement, capacity + advance-notice enforcement, an unpaid Payment row per order/booking) are built; there's no staff-invitation flow, no service-time-slot management endpoint, and no order/booking status-transition (confirm/cancel/mark-paid) endpoints yet. Check `database/migrations/`, `app/Models/`, and `routes/api.php` for the current state rather than assuming this doc stays in sync.
...
- `App\Http\Middleware\EnsureTenantAccess` (aliased `tenant.access`, only on catalog write routes, after `auth:sanctum` + `tenant`) and the visibility check inside `OrderController`/`BookingController` both call `Tenant::isManagedBy(User $user)`: true for the platform admin, the tenant's `owner_user_id`, or an active `tenant_staff` row for that (tenant, user) pair. There's no staff-invitation endpoint yet, so in practice only the owner or a platform admin manages a tenant today.
...
- Both `store` methods create an associated `Payment` (`status: unpaid`) for the customer-chosen `payment_method` — there's no real payment gateway, `PaymentMethod` is pay-in-person/manual options only, and nothing yet flips a payment to `paid` (no endpoint for that either).
