...
This is a Laravel 12 application. The central + tenant-schema database (see "Database architecture" below), Sanctum SPA auth, role-gated admin CRUD for tenants/business types, tenant provisioning, a tenant-scoped product/service catalog, order/booking placement, and order/booking/payment fulfillment (status transitions, mark-paid) are built; there's no staff-invitation flow and no service-time-slot management endpoint yet. Check `database/migrations/`, `app/Models/`, and `routes/api.php` for the current state rather than assuming this doc stays in sync.
